...
const QuickMenu = React.forwardRef(function QuickMenu({ items, onSelect, style, anchorRef }, ref) {
  ...
  useLayoutEffect(() => {
    ...
    if (anchorRef && anchorRef.current) {
      const aw = anchorRef.current.offsetWidth;
      if (aw) {
        setLimitStyle({ maxWidth: aw });
      }
    }
  ...
  return (
    <div
      ...
      style={{ position: 'absolute', background: '#fff', border: '1px solid #ccc', borderRadius: 4, zIndex: 50, overflowX: 'auto', ...style, ...limitStyle, ...adjust }}
    >
      {items.length > 0 ? (
        items.map(item => (
          <div
